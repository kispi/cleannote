<script lang="ts">
  ...
  let { lat, lng, name, apiName, placeId }: Props = $props()

  let displayName = $derived(apiName || name)
  ...
  let apps = $derived([
    {
      ...
      url: `https://map.naver.com/p/search/${encodeURIComponent(displayName)}`
    },
    {
      id: 'kakao',
      name: 'Kakao Map',
      ...
      url: placeId
        ...
        : `https://map.kakao.com/link/map/${displayName},${lat},${lng}`
    },
    {
      id: 'google',
      name: 'Google Maps',
      ...
      color: 'bg-white border border-base text-[#191919] hover:bg-gray-50',
...
<div class="modal-navigation bg-base-100 relative w-full rounded-3xl p-6 text-center">
  <h3 class="text-base-content mb-6 text-lg font-bold">
    {t('navigation.title')}
  </h3>
  ...
  <div class="grid gap-3">
    {#each apps as app}
      <button
        onclick={() => openApp(app)}
        class="flex items-center gap-4 rounded-xl border p-4 transition-all active:scale-95 {app.color}"
      >
        <span
          class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white/20 text-lg font-bold"
        >
          {@html app.iconSvg}
        </span>
  ...
  <button
    ...
    class="btn-default mt-6 w-full rounded-xl py-3 font-bold"
  >
    {t('common.cancel')}
  </button>
</div>
